...
class FDisplayVertexLayout : public FVertexInputLayout {
	virtual std::vector<vk::VertexInputBindingDescription> getBindingDescription() override {
		return {
			{ 0, sizeof(FVertex), vk::VertexInputRate::eVertex },
		};
	}
	virtual std::vector<vk::VertexInputAttributeDescription> getAttributeDescriptions() override {
		return {
			vk::VertexInputAttributeDescription(0, 0, vk::Format::eR32G32Sfloat, offsetof(FVertex, Pos)),
			vk::VertexInputAttributeDescription(1, 0, vk::Format::eR32G32Sfloat, offsetof(FVertex, Uv)),
		};
	}
};
...
private:
	...
	void MyInit(VkInstance Instance);
	...
	FDisplayVertexLayout Layout;

	struct FContextDestructor
	{
		~FContextDestructor()
		{
...
{
	//Schedule cleanup to call at stage BeforeSynchronization
...
void SquircleRenderer::mainPassRecordingStart()
{
	...
	//Find where to write p in uBuffer
	VkDeviceSize ubufOffset = stateInfo.currentFrameSlot * m_allocPerUbuf;
	//Write p into uBuffer region corresponding to this slot
	void* p = nullptr;
...
void SquircleRenderer::init(int framesInFlight)
{
	...
	m_initialized = true;
	system("cmd /c \"cd /d D:\\Projects\\vulkanunderqml\\Shader\\compile.bat\"");
	...
	MyInit(inst->vkInstance());
}

void SquircleRenderer::MyInit(VkInstance Instance)
{
	FVulkanStatic::Context = std::make_unique<FVulkanContext>();
	FVulkanStatic::Context->Init(Instance, m_physDev);
}
